  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
      <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
        ...
        <p className="font-bold text-gray-900 mt-1">{batch.companyRuc}</p>
        <p className="text-[10px] text-gray-500 italic truncate">{batch.fileName}</p>
      </div>
      <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tipo de Servicio</p>
        <p className="font-bold text-gray-900 mt-1">NOM</p>
      </div>
      <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
        ...
        <p className="font-mono text-sm font-bold text-blue-600 mt-1">
          {sessionStorage.getItem(`account_${batch.batchId}`) || 'CTA-CORRIENTE-VINCULADA'}
        </p>
      </div>
      <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Monto Declarado</p>
        <p className="text-xl font-black text-gray-900 mt-1">
          ${(batch.controlAmount || 0).toLocaleString('es-EC', { minimumFractionDigits: 2 })}
        </p>
      </div>
      <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Registros</p>
        <p className="text-xl font-black text-gray-900 mt-1">{batch.totalRecords}</p>
      </div>
      <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm">
